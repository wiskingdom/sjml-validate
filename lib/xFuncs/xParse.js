...
const lineParse = container => {
  ...
  return inputXml.split(/[\n\r]+/).reduce((acc, curr, index) => {
    if (curr.match(/^\s*?<.+<\//)) {
      ...
      const parseLog = e ? [[inputFileName, index + 1, e.code, e.msg, curr]] : [];
      return [...acc, ...parseLog];
    }
    return acc;
  }, []);
...
const seParse = container => {
  ...
  const newXml = inputXml.split(/[\n\r]+/).map(line => {
    if (m = line.match(/(.*<head stage=")(.+)(" ?\/>.*)/)) {
      return m[1] + m[2].replace(/>/g, '&gt;') + m[3];
    } else {
      return line;
    }
  }).join('\n');
  const parsedContainer = wParse({ ...container, inputXml: newXml });
  const { obj, passParse } = parsedContainer;
 
  if (passParse) {
    const { episode } = obj.SJML.text;
    obj.SJML.text.episode = Array.isArray(episode)? episode : [episode];
    obj.SJML.text.episode = obj.SJML.text.episode.map(epItem => {
...
const parseGroup = xml => {
  const sjmls = xml.trim().split('</SJML>').slice(0, -1).map(xml => {
    ...
    const p = Array.isArray(SJML.text.p) ? SJML.text.p : [SJML.text.p];
    return { ...SJML, text: { p } };
  });
...
const eParse = container => {
  ...
  const obj = passParse ? parseGroup(inputXml) : {};
  return { ...container, obj, passParse, parseLog };
};
...
const xParse = container => {
  ...
  if (parsedContainer.obj.SJML) {
    ...
    const fileId = header.fileInfo ? header.fileInfo.fileId : '';
