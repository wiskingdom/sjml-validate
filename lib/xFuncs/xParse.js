...
const getSJML = obj => {
  const SJML = Array.isArray(obj.SJMLGROUP.SJML) ? obj.SJMLGROUP.SJML: [obj.SJMLGROUP.SJML];
  obj.SJMLGROUP.SJML = SJML;
  return obj;
};
...
const eParse = container => {
  ...
  if (passParse) {
    ...
    const SJML = getSJML(obj).SJMLGROUP.SJML.map(item => {
      let p = [];
      if (!Array.isArray(item.text.p) ) {
        p = [item.text.p];
      }
      item.text.p = p;
      return item;
