...
const toXLSX = schemaType => report => {
  const { parseLog, validLog, tokenSizeResult, processResult, filePathMap } = report;
  ...
  if (['SDRW', 'SXRW'].includes(schemaType)) {
    const { extraLog } = report;
    const extraHeader = ['fileName', 'type'];
    const tagTypeSheet = XLSX.utils.aoa_to_sheet([extraHeader, ...extraLog])
    XLSX.utils.book_append_sheet(wb, tagTypeSheet, 'tag_in_u');
  }
...
const xLog = ({ outputFolder, runType, schemaType, report, date }) => {
  ...
  const wb = toXLSX(schemaType)(report);
