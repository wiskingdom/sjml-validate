    ...
    if (acc.toggle) {
      if (line.match(/^  <\/text>/)) {
        ...
        if (['EXRW'].includes(schemaType) && (m = line.match(/(.*?)<p>(.*?)<\/p>/))) {
          acc.lines.push(`${m[1]}<p>${xmlEscape(m[2])}</p>`);
        } else {
    ...
    if (acc.toggle) {
      if (line.match(/^  <\/header>/)) {
        ...
        if (m = line.match(/(.*?)<title>(.*?)</)) {
          acc.lines.push(`${m[1]}<title>${xmlEscape(m[2])}</title>`);
        } else if (m = line.match(/(.*?)<author>(.*?)</)) {
          acc.lines.push(`${m[1]}<author>${xmlEscape(m[2])}</author>`);
        } else if (m = line.match(/(.*?)<publisher>(.*?)</)) {
          acc.lines.push(`${m[1]}<publisher>${xmlEscape(m[2])}</publisher>`);
        } else if (m = line.match(/(.*?)<url>(.*?)</)) {
          acc.lines.push(`${m[1]}<url>${xmlEscape(m[2])}</url>`);
        } else {
          acc.lines.push(line);
        }
      }
