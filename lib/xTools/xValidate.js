...
const checkSideChar = ({ u, reportFileName }) => {
  ...
  return sideChar ? [['sideChar', reportFileName, sideChar]] : false;
};

const checkInnerSpace = ({ u, reportFileName }) => {
  const innerSpace = u.find(elem => elem.cdata.match(/ <\//));
  return innerSpace ? [['innerSpace', reportFileName, innerSpace]] : false;
};
...
const checkAnon = ({ u, reportFileName }) => {
  ...
  if (noIdName && IdName) {
    const m = IdName.match(/<anon type="name" n=(.+?)\/>/);
    const nameId = m[1] || '';
...
const xValidate = container => {
  ...
  if (!valid) {
    ...
    if (orderErrorIdx > -1) {
      validLog = [
        reportFileName,
        `.SJML.text.u[${orderErrorIdx}].att_n`,
