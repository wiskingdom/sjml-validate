...
const toXLSX = ({ runType, report }) => {
  ...
  const parseLogSingle = getSingleLogByfile(parseLog);
  const pLogSheet = XLSX.utils.aoa_to_sheet([pLogHeader, ...parseLogSingle]);
...
const xsLog = ({ outputFolder, runType, schemaType, report, date }) => {
  ...
  if (report.parseLog.length) {
    fs.writeFileSync(
      path.join(outputFolder, parseAll),
      report.parseLog.map(row => row.join('\t')).join('\n'),
      'utf8'
    );
  }
